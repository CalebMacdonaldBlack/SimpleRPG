...
public class Slowness extends Modifier {
	...
	public Slowness(Main plugin, String name, double rate, int duration, Long interval, double intensity, String type,
		...
		if (getTarget().equals("player")) {
			if (getTrigger().equals("hold") || getTrigger().equals("have"))
				player.addPotionEffect(
						new PotionEffect(PotionEffectType.SLOW, Integer.MAX_VALUE, (int) getIntensity()));
			else
				player.addPotionEffect(
						new PotionEffect(PotionEffectType.SLOW, getDuration() * 20, (int) getIntensity()));
		} else {
