...
public class Poison extends Modifier{
	...
	public Poison(Main plugin, String name, double rate, Long duration, Long interval, double intensity, String type,
	...
	public void applyBuff(Player player, Entity entity) {
		if(getTarget().equals("player")){
			if(getTrigger().equals("hold"))
				player.addPotionEffect(new PotionEffect(PotionEffectType.POISON, Integer.MAX_VALUE, (int) getIntensity()));
			else
				player.addPotionEffect(new PotionEffect(PotionEffectType.POISON, (int) (getDuration()/1000), (int) getIntensity()));
		}else{
			if (entity instanceof LivingEntity) {
				((LivingEntity) entity).addPotionEffect(new PotionEffect(PotionEffectType.POISON, (int) (getDuration()/1000*20), (int) getIntensity()),true);
